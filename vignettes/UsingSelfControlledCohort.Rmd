...
We can see that `example$numPersons` were exposed to amoxicillin. When
we add up all the time before the exposure we get
`example$timeAtRiskUnexposed` person-years. Similarly when we add up
all the time after the exposure, the "Exposed time at risk", we get
`example$timeAtRiskExposed` person-years.
